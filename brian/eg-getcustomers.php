...
//if(isset($_POST["salesrep"]) && !empty($_POST["salesrep"])){
	...
	$urlx = "https://3743011.restlets.api.netsuite.com/app/site/hosting/restlet.nl?script=customscriptgsi_r_brian_customers&deploy=1&salesrep=" . $_POST["salesrep"];
	...
	if ($_SESSION['usertype'] == "dealer"){
		...
		foreach($data as $row) {
			if($row['id'] == $_SESSION['dealer']) {
				$_SESSION['dealer_name'] = $row['name'];
				if ($row['credithold'] == 'hold'){
					echo '<option disabled="disabled" value="'.$row['id'].'" selected="selected>'.$row['name'].'"</option>';
				}
				else {
