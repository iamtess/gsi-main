...
//if(isset($_POST["salesrep"]) && !empty($_POST["salesrep"])){
	...
	$urlx = "https://3743011.restlets.api.netsuite.com/app/site/hosting/restlet.nl?script=customscriptgsi_r_brian_salesreps&deploy=1";
	...
	if ($_SESSION['usertype'] == "dealer"){
		...
		foreach($data as $row) {
			if($row['id'] == $_SESSION['tm']) {
				$_SESSION['tm_name'] = $row['name'];
