	...
	$price = '';
		...
		if ($_POST["medVolume"] > 0){
			...
			foreach($data as $row) {
				...
				if ($country == 'au'){
					if ($budgetU == 'all'){
						...
						elseif ($row['rrpus'] > 499.00 && $row['rrpus'] < 800.00){
							$budget = '$$';
						}
						else {
							$budget = '$$$';
				...
				elseif  ($country == 'nz'){
					...
					elseif ($row['rrpus'] > 425.00 && $row['rrpus'] < 650.00){
						$budget = '$$';
					}
					else {
						$budget = '$$$';
			...
			if ($flag == ''){
				...
				echo "Sorry there are no boards for the $budgetU budget in your volume range. Please try a different budget.";
			}
			echo '</div>';
			echo '</div>';
			echo '</div>';
		...
		echo '<section class="pt0">';
            ...
            echo '<div class="row">';
			echo '<div class="col-md-12 col-sm-10">';
			echo '<p>Sorry there are no boards in this volume range.</p>';
			echo '</div>';
			echo '</div>';
			echo '</div>';
			echo '</section>';
